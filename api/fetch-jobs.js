...
async function fetchSupabase({ url, anonKey, where, q, days, limit }) {
  ...
  if (q) params.set('or', `(title.ilike.*${q}*,description.ilike.*${q}*,company.ilike.*${q}*)`);
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // Fetch in parallel; tolerate one source failing
    ...
    return okJson(res, { ok: true, ms: Date.now() - t0, sources, jobs: finalJobs }, 200);
